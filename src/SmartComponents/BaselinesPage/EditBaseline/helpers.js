...
function renderKebab(factName, valueName, fact) {
    return <td>
        { <FactKebab
            factName={ factName }
            valueName={ valueName }
...
function buildNewFactData(isParent, factName, valueName, factData) {
    let newFactData = {};
    let subFacts = [];
    ...
    if (factData && !Array.isArray(factData)) {
        ...
        subFacts.push({ name: factName, value: valueName });
    ...
    if (isParent) {
        ...
        newFactData.value = valueName;
    }

    return newFactData;
}

function buildEditedFactData(isParent, originalFactName, factName, originalValueName, valueName, factData) {
    let newAPIBody = {};
    let subFacts = [];
    ...
    if (isParent) {
        ...
        if (factData.values && valueName !== '') {
            newAPIBody.name = factData.name;
            let newSubFact = { name: factName, value: valueName };
            ...
            newAPIBody = { name: factName, value: valueName };
        }
    }
...
function makeAPIPatch(data, originalAPIBody, originalParentFact) {
    ...
    if (originalParentFact.length === 0) {
