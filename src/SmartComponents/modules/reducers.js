...
export function compareReducer(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case `${types.FETCH_COMPARE}_FULFILLED`:
            filteredFacts = reducerHelpers.filterCompareData(action.payload.facts, state.stateFilters, state.factFilter, state.expandedRows);
        ...
        case `${types.UPDATE_PAGINATION}`:
            filteredFacts = reducerHelpers.filterCompareData(state.fullCompareData, state.stateFilters, state.factFilter, state.expandedRows);
            sortedFacts = reducerHelpers.sortData(filteredFacts, state.factSort, state.stateSort);
        ...
        case `${types.ADD_STATE_FILTER}`:
            ...
            filteredFacts = reducerHelpers.filterCompareData(state.fullCompareData, updatedStateFilters, state.factFilter, state.expandedRows);
            sortedFacts = reducerHelpers.sortData(filteredFacts, state.factSort, state.stateSort);
            paginatedFacts = reducerHelpers.paginateData(sortedFacts, 1, state.perPage);
            return {
                ...state,
        ...
        case `${types.FILTER_BY_FACT}`:
            filteredFacts = reducerHelpers.filterCompareData(state.fullCompareData, state.stateFilters, action.payload, state.expandedRows);
            sortedFacts = reducerHelpers.sortData(filteredFacts, state.factSort, state.stateSort);
            paginatedFacts = reducerHelpers.paginateData(sortedFacts, 1, state.perPage);
            return {
                ...state,
        ...
        case `${types.EXPAND_ROW}`:
            ...
            filteredFacts = reducerHelpers.filterCompareData(state.fullCompareData, state.stateFilters, state.factFilter, newExpandedRows);
            sortedFacts = reducerHelpers.sortData(filteredFacts, state.factSort, state.stateSort);
